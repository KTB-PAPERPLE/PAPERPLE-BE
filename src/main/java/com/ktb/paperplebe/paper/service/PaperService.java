...
public class PaperService {
  ...
  public List<UserPaperResponse> getMyPapers(Long userId) {
    ...
    return papers.stream()
        .map(
            paper ->
                UserPaperResponse.of(
                    paper, user.getNickname(), user.getProfileImage(), Optional.empty()))
        .collect(Collectors.toList());
  }
    ...
    return papers.stream()
        .map(
            paper -> {
              ...
              return UserPaperResponse.of(
                  paper,
                  paper.getUser().getNickname(),
                  paper.getUser().getProfileImage(),
                  Optional.of(isLikedByCurrentUser));
            })
        .collect(Collectors.toList());
  }
  ...
  private Paper findPaperByIdOrThrow(Long paperId) {
    return paperRepository.findById(paperId).orElseThrow(() -> new PaperException(PAPER_NOT_FOUND));
  }
}
